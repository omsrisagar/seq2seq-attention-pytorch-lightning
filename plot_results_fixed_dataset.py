...
if __name__ == "__main__":
    ...
    parser.add_argument("--last_epoch1", type=int, default=50, help="Used for assertion - to make sure training ran this many epochs")
    ...
    training_files = ['ar-training-data_60606060_31250.txt']
    ...
    usecols_dict = {key : np.zeros((len(model_names), len(dim_levels), len(dim_levels[0]))) for key in usecols if key != 'epoch'}
    metrics_desc_dict = dict(zip(metrics, metrics_desc))

    # for gathering data use usecols
    for i in range(len(model_names)):
        for j in range(len(dim_levels)):
            for k in range(len(dim_levels[0])):
                logdir = Path(args.base_folder, model_names[i], f'{dim_names[j]}_{dim_levels[j][k]}')
                ckpt_dir = Path(logdir, Path(training_files[i]).stem, 'csv_logs')
                ...
                for key, value in usecols_dict.items():
                    ...
                    if 'train' in key:
                        assert (df['epoch'].iloc[indx] == args.last_epoch - 1) or \
                               (df['epoch'].iloc[indx] == args.last_epoch1 - 1) # make sure training ran till this point
                    else:
                        assert (df['epoch'].iloc[indx] == args.last_epoch) or \
                               (df['epoch'].iloc[indx] == args.last_epoch1)
                    value[i][j][k] = df[key].iloc[indx]


    # for plotting use metrics
    for i in range(len(dim_levels)):
        output_folder = Path(args.base_folder, 'results', model_names[0], f'{args.ckpt_to_use}_checkpoint')
        output_folder.mkdir(parents=True, exist_ok=True)
        with pd.ExcelWriter(f'{output_folder}/{dim_names[i]}.xlsx') as writer:
            for key in metrics:
                if args.write_results:
                    df = pd.DataFrame(usecols_dict[key][:,i,:], columns=dim_levels[i])
                    # df = pd.DataFrame(usecols_dict[key][:,i,:], columns=dim2_levels, index=legend_model_names)
                    # df.to_csv(f'{output_folder}/{key}.csv', index=True, header=True)
                    df.to_excel(writer, sheet_name=metrics_desc_dict[key], index=False, header=True)
                    if 'loss' not in key and 'sequence' not in key: # write error information as well
                        df = pd.DataFrame(usecols_dict[err_dict[key]][:, i, :], columns=dim_levels[i])
                        # df = pd.DataFrame(usecols_dict[err_dict[key]][:, i, :], columns=dim2_levels, index=legend_model_names)
                        # df.to_csv(f'{output_folder}/{key}_err.csv', sep='\t', index=True, header=True)
                        df.to_excel(writer, sheet_name=metrics_desc_dict[key] + ' Error', index=False, header=True)
                plot_figures(
                    output_path=f'{output_folder}/{dim_names[i]}',
                    desc=key,
